...
export function DataTable({
    ...
    async function getListWithCurrentOptions(
        page = currentPage,
        limit = 10
    ) {
        setLoading(true);
        ...
        if (searchRoute) {
            await axios.get(`/api/${searchRoute}?${generateQueryFromObject(requestParams)}`, {
                headers: {
                    "Accept": "text/json",
                },
            })
                .then((data) => {
                    const { data: tableData, meta } = data.data;
                    if (resource == true) {
            ...
            case 'multi-badge':
                ...
                return <div className="flex flex-wrap gap-1">
                    {value.map((item, index) => (
                        <Badge key={index} variant={variant ?? 'default'}>{item.name}</Badge>
                    ))}
                </div>
            case 'link':
                return <a
    ...
    }, [order, orderBy, filtersValues, debouncedToSearch]);
                        ...
                        <TableHeader>
                            <TableRow>
                                {Object.keys(columnMap).map((field) => (
                                    ...
                                    >
                                        <div className={`${columnMap[field].disableSort ? '' : 'cursor-pointer'} select-none flex gap-2 items-center ${columnMap[field].align ? 'justify-' + columnMap[field].align : 'justify-start'} text-center`}>
