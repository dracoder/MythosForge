...
import genreFormSchema from '@/validations/genreFormSchema';
...
export default function EditForm({ onSubmit, open, onOpenChange, editingId }) {
    ...
    const { register, formState: { errors }, handleSubmit, watch, reset, control, setError, setValue } = useForm({
        ...
        resolver: yupResolver(genreFormSchema(t))
    });
    ...
    useEffect(() => {
        if (!editingId) return;
        GenreService.show(editingId, (data) => {
            reset(data.data);
        });
    }, [editingId]);

    const handleUpdateAction = async (data) => {
        await GenreService.update(editingId, data)
            .then((res) => {
                toast.success(t('pages.updated', { name: t('pages.genre') }));
                setLoading(false);
                reset();
                onOpenChange(false);
                onSubmit();
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{t('pages.edit', { name: t('pages.genre') })} #{editingId}</DialogTitle>
                </DialogHeader>
