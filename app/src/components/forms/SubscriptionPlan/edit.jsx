...
import subscriptionPlanFormSchema from '@/validations/subscriptionPlanFormSchema';
...
export default function EditForm({ onSubmit, open, onOpenChange, editingId }) {
    ...
    const { register, formState: { errors }, handleSubmit, watch, reset, control, setError, setValue } = useForm({
        ...
        resolver: yupResolver(subscriptionPlanFormSchema(t, true))
    });
    ...
    useEffect(() => {
        if (!editingId) return;
        SubscriptionPlanService.show(editingId, (data) => {
            reset(data.data);
        });
    }, [editingId]);

    const handleUpdateAction = async (data) => {
        await SubscriptionPlanService.update(editingId, data)
            .then((res) => {
                toast.success(t('pages.updated', { name: t('pages.subscription_plan') }));
                setLoading(false);
                reset();
                onOpenChange(false);
                onSubmit();
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{t('pages.edit', { name: t('pages.subscription_plan') })} #{editingId}</DialogTitle>
                </DialogHeader>
