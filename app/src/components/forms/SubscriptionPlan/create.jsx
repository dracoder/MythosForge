...
export default function CreateForm({ onSubmit, open, onOpenChange }) {
    ...
    const { register, formState: { errors }, handleSubmit, watch, reset, control, setError, setValue } = useForm({
        ...
        resolver: yupResolver(subscriptionPlanFormSchema(t))
    });
    ...
    const handleStoreAction = async (data) => {
        setLoading(true);
        await SubscriptionPlanService.store(data)
            .then((res) => {
                toast.success(t('pages.created', { name: t('pages.subscription_plan') }));
                setLoading(false);
                reset();
                onOpenChange(false);
                onSubmit();
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{t('pages.create', { name: t('pages.subscription_plan') })}</DialogTitle>
                </DialogHeader>
